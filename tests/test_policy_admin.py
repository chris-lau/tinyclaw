...
def test_pack_seeds_db_set_on_boot(tmp_path: Path) -> None:
    with make_client(tmp_path) as c:
        r = c.get("/api/sets/procurement/policy")
...
def test_edit_validates_versions_and_audits(tmp_path: Path) -> None:
    with make_client(tmp_path) as c:
        ...
        bad = c.put("/api/sets/procurement/policy", json={"yaml": "policies: [oops"})
        assert bad.status_code == 422
        # empty rules rejected
        bad2 = c.put("/api/sets/procurement/policy", json={"yaml": "policies: []\n"})
        assert bad2.status_code == 422

        original = c.get("/api/sets/procurement/policy").json()["yaml"]
        edited = original.replace("value: 5000", "value: 9000")  # tier-2 threshold moves
        r = c.put("/api/sets/procurement/policy", json={"yaml": edited, "updated_by": "pm"})
        assert r.status_code == 200
        assert r.json()["version"] == 2 and r.json()["rules"] >= 5

        # the saved set evaluates with the new threshold
        t = c.post(
            "/api/sets/procurement/policy/test",
        ...
        # audited
        audit = c.get("/api/audit", params={"action": "config.update"}).json()
...
def test_dry_run_on_draft_without_saving(tmp_path: Path) -> None:
    with make_client(tmp_path) as c:
        ...
        # saved set untouched
        assert c.get("/api/sets/procurement/policy").json()["version"] == 1
...
def test_audit_filters(tmp_path: Path) -> None:
    with make_client(tmp_path) as c:
        ...
        c.post(
            "/internal/audit",
            json={"actor": "human:pm", "action": "config.update", "subject": "procurement", "decision": "v2"},
            headers=token,
        )

        assert all(a["actor"] == "human:pm" for a in c.get("/api/audit", params={"actor": "human"}).json())
        assert len(c.get("/api/audit", params={"action": "config.update"}).json()) == 1
