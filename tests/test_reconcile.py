...
def _park_and_decide(c: TestClient, title: str) -> str:
    ...
    c.post(
        "/internal/audit", json={"actor": "gateway", "action": "approval.requested", "subject": task_id}, headers=token
    )
    ...
    pending = [a for a in c.get("/api/approvals?status=pending").json() if a["task_id"] == task_id]
    r = c.post(f"/api/approvals/{pending[0]['id']}/decision", json={"decision": "approve", "approver": "t"})
